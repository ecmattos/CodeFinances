<template>
	<!--<div class="container">-->
		<div class="row">
			...
			<div class="fixed-action-btn">
				<a class="btn-floating btn-large red" v-link="{name: 'bank-account.create'}">
	...
	<modal :modal="modal">
		<div slot="content" v-if="bankAccountDelete">
			...
			<p>Conta: <strong>{{ bankAccountDelete.account }}</strong></p>
			<div class="divider"></div>
		</div>
		<div slot="footer">
		...
		methods: {
			destroy() {
				store.dispatch('bankAccount/delete').then((response) => {
					Materialize.toast('Conta bancária excluída com sucesso !', 4000, 'rounded red');
				});
			},
