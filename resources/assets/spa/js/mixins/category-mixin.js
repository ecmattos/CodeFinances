    ...
    methods: {
        saveCategory(){
            store.dispatch(`${this.namespace()}/save`, this.categorySave).then(response => {
                if (this.categorySave.id === 0) {
                    Materialize.toast('Categoria adicionada com sucesso!', 4000, 'rounded green');
                } else {
                    Materialize.toast('Categoria alterada com sucesso!', 4000, 'rounded blue');
                }
                this.resetScope();
            });
        },
        destroy(){
            store.dispatch(`${this.namespace()}/delete`, this.categorySave)
                .then(response => {
                    Materialize.toast('Categoria excluida com Sucesso', 4000, 'rounded red');
                })
        },
        modalNew(category){
