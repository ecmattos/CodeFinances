...
const actions = {
    ...
    update(context, bankAccount){
    	return BankAccount.update({id: bankAccount.id}, bankAccount).then((response) => {
    		return response;
        });
    },
    get(context, id){
    	return BankAccount.get({id: id, include: 'bank'}).then((response) => {
        	return response;
        });
    }
};
